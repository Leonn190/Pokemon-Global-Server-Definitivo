"""Projétil Pokébola para captura de Pokémon no mundo."""
...
class Pokebola(Projetil):
    """Projétil que tenta capturar um ``PokemonMundo`` ao colidir."""
    ...
    def _ao_colidir_entidade(self, projetil: "Pokebola", entidade) -> None:
        if isinstance(entidade, PokemonMundo):
            entidade.capturar(capturador_id=self.TreinadorId)
            self.morrer()

...
class PokemonMundo(Entidade):
    ...
    # ---------------------------------------------------------
    # ESCALA DOS FRAMES (PROPORCIONAL)
    # ---------------------------------------------------------
    ...
    def _obter_frames_escalados(cls, especie: str, tamanho_px: int) -> List[pygame.Surface]:
        ...
        for frame in frames:
            ...
            try:
                frame_escalado = pygame.transform.smoothscale(frame, (novo_w, novo_h))
                frames_escalados.append(frame_escalado)
            except Exception:
                continue
    ...
    def aplicar_snapshot(self, snapshot: Dict[str, object]) -> None:
        ...
        self.Colisor.raio_colisao = max(
            0.2,
            self._f(snapshot.get("raio_colisao"), self.Colisor.raio_colisao),
        )
        ...
        self._velocidade_interp_tiles_s = max(
            1.2,
            min(3.2, 1.2 + (vel_stats / 220.0)),
        )

        movimento = str(
            snapshot.get("movimento")
            or estado.get("movimento")
            or "mover"
        ).strip().lower()
    ...
    def desenhar(self, tela, camera, dt: float) -> None:
        ...
        pulsar = 1.0 + math.sin(
            t + (abs(hash(self.Nome)) % 360) * 0.017
        ) * 0.06
        ...
        frames = self._obter_frames_escalados(self.Especie, sprite_tamanho)

        if frames:
            idx = int((pygame.time.get_ticks() / 100) % len(frames))
            frame = frames[idx]

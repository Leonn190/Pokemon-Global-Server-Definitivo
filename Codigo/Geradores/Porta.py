...
def esta_perto(pos_player, pos_porta, raio=2.0):
    return distancia_quadrada(pos_player, pos_porta) <= float(raio) ** 2
...
def renderizar(tela, camera, posicao, modo="dungeon", **kwargs):
    ...
    if modo == "estadio":
        ...
        px_porta = int(kwargs.get("px_porta", x)); py_porta = int(kwargs.get("py_porta", y))
        px = lambda v: max(1, int(v * tile))
        porta_w = max(px(1.7), 34); porta_h = max(px(2.4), 52)
        porta_externa = pygame.Rect(0, 0, porta_w, porta_h); porta_externa.midbottom = (px_porta, py_porta)
        ...
        pygame.draw.ellipse(tela, (68, 79, 100), arco); pygame.draw.ellipse(tela, (44, 50, 64), arco, max(2, px(0.07)))
        ...
        if brilho.width > 4 and brilho.height > 4: pygame.draw.rect(tela, (190, 230, 255), brilho, border_radius=max(4, px(0.08)))

...
class ControladorObjetos:
    ...
    def _dimensao_player_local(self) -> str:
        ...
        payload = self.ObjetosPorId.get(pid, {}) if (pid > 0 and isinstance(self.ObjetosPorId, dict)) else {}
        estado = payload.get("estado") if isinstance(payload.get("estado"), dict) else {}
        dim_payload = str(estado.get("dimensao") or payload.get("dimensao") or "").strip()

        ref = self._player_local_ref
        dim_ref = str(getattr(ref, "DimensaoAtual", "") or "").strip() if ref is not None else ""

        if dim_payload and dim_ref and dim_payload != dim_ref:
            registrar_evento_npc_estadio("cliente_dimensao_divergente", dimensao_ref=dim_ref, dimensao_payload=dim_payload, player_id=pid)
            setattr(ref, "DimensaoAtual", dim_payload)
            return dim_payload
        if dim_payload:
            return dim_payload
        if dim_ref:

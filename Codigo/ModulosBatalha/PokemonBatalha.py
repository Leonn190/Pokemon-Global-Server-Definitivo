...
class PokemonBatalha:
    ...
    def atualizar_animacao(self, dt: float):
        ...
        self.TimerAnimacao += dt * multiplicador
        tempo_frame = max(0.01, float(self.TempoFrame or 0.01))
        while self.TimerAnimacao >= tempo_frame:
            self.TimerAnimacao -= tempo_frame
    ...
    def _aplicar_transformacao_visual(self, img):
        out = img
        rotacao = float(self.RotacaoVisual or 0.0) + self._rotacao_efeitos_dinamicos()
        escala = self._escala_efeitos_dinamicos()
        if abs(rotacao) > 0.001 or abs(escala - 1.0) > 0.001:
            out = pygame.transform.rotozoom(out, rotacao, escala)
        out = self._aplicar_filtros_efeitos(out)
        alpha = max(0, min(255, int(self.AlphaVisual * self._multiplicador_alpha_efeitos())))
        if alpha < 255:
            out = out.copy()
            out.set_alpha(alpha)
        return out

...
class Pokemon:
    ...
    def aplicar_snapshot(self, snapshot: Dict[str, object]) -> None:
        ...
        tomper_bonus_barra = self._f(self.EstadoFrutificacao.get("bonus_tamanho_barra_captura_percentual"), 0.0)
        abbajuur_mult_velocidade = self._f(self.EstadoFrutificacao.get("multiplicador_velocidade_barra_captura"), 1.0)
        self.TamanhoBarraCaptura = max(0.06, min(0.45, tamanho_barra * (1.0 + tomper_bonus_barra / 100.0)))
        self.VelocidadeBarraCaptura = max(20.0, min(260.0, velocidade_barra * max(0.05, abbajuur_mult_velocidade)))

...
class FichaAtaque:
    ...
    def renderizar(self, tela: pygame.Surface, rect, ataque: dict | None, atributos: dict | None = None):
        ...
        if fundo is not None:
            header_surf = pygame.Surface(header.size, pygame.SRCALPHA)
            header_surf.blit(fundo, fundo.get_rect(center=header_surf.get_rect().center))
            mascara = pygame.Surface(header.size, pygame.SRCALPHA)
            pygame.draw.rect(mascara, (255, 255, 255, 255), mascara.get_rect(), border_radius=12)
            header_surf.blit(mascara, (0, 0), special_flags=pygame.BLEND_RGBA_MULT)
            sombra = pygame.Surface(header.size, pygame.SRCALPHA)
            pygame.draw.rect(sombra, (0, 0, 0, 76), sombra.get_rect(), border_radius=12)
            header_surf.blit(sombra, (0, 0))
            tela.blit(header_surf, header.topleft)
        else:

...
def TelaMenu(JOGO, EVENTOS, dt):
    if not hasattr(JOGO, "EstadoTelaMenu"):
    ...
    estado = JOGO.EstadoTelaMenu
    largura_tela, altura_tela = JOGO.TELA.get_size()
    max_offset = max(0, estado["fundo_largura"] - largura_tela)

    if max_offset > 0:
        estado["fundo_offset_x"] += estado["fundo_velocidade"] * dt * estado["fundo_direcao"]
        if estado["fundo_offset_x"] >= max_offset:
            estado["fundo_offset_x"] = float(max_offset)
            estado["fundo_direcao"] = -1
        elif estado["fundo_offset_x"] <= 0:
            estado["fundo_offset_x"] = 0.0
            estado["fundo_direcao"] = 1

    JOGO.TELA.blit(estado["fundo"], (-int(estado["fundo_offset_x"]), 0))

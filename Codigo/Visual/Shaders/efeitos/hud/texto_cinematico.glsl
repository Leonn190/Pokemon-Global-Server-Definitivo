vec3 aplicar_texto_cinematico_hud(vec3 color, vec4 hud, vec2 screen_uv) {
    ...
    float alpha_perto = hud_alpha_blur(v_uv, 7.0);
    float alpha_longe = hud_alpha_blur(v_uv, 18.0);
    float texto_mask = max(hud.a, alpha_perto);
    if (texto_mask <= 0.003) {
        return color;
    }
    float texto_alpha = clamp(hud.a + alpha_perto * 0.55, 0.0, 1.0);
    ...
    float borda = max(alpha_longe - alpha_perto * 0.35, 0.0) * (1.0 - hud.a);

...
CAMINHO_ATUALIZADOR_README_ATUAL = "Ferramentas/AtualizadorReadMe.py"
...
def enriquecer_relatorio_atualizado(
    ...
    *,
    ...
    meta["imagens_dir"] = f"Documentação/Relatorios/Imagens/{stem_original}"
    ...
    if isinstance(graficos, dict):
        for chave, valor in list(graficos.items()):
            if not isinstance(valor, str):
                continue
            valor = valor.replace(
                f"Documentação/Relatorios/Imagens/{stem_gerado}/",
                f"Documentação/Relatorios/Imagens/{stem_original}/",
            )

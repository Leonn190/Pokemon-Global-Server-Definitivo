# Registro

**Relatório:** #31  
**Repo:** `Pokemon-Global-Server-Definitivo`  
**Gerado em:** 2026-04-10T16:33:05  

## Visão geral

- **Pastas:** 1.183
- **Arquivos:** 69.042
- **Tamanho total:** 433.667.625 bytes (0.404 GiB)
- **Linhas totais gerais:** 44.008
- **Commits (repo):** 309

## Python

- **Arquivos `.py`:** 140
- **Linhas totais:** 35.235
- **Classes encontradas:** 130
- **Funções encontradas:** 385
- **Métodos encontrados:** 1.622
- **Total funções + métodos:** 2.007
- **Média de linhas por arquivo `.py`:** 251.68

### Rank das 10 pastas mais importantes por linhas

Contagem recursiva: cada pasta inclui todas as subpastas e todos os arquivos internos.

| Rank | Pasta | Caminho | Subpastas | Arquivos | Linhas gerais | `.py` | `.json` | `.toml` | Outras |
|---:|---|---|---:|---:|---:|---:|---:|---:|---:|
| 1 | `SimuladorServerJogo` | `SimuladorServerJogo` | 6 | 55 | 11.554 | 7.704 | 658 | 357 | 2.835 |
| 2 | `Telas` | `Codigo/Telas` | 1 | 18 | 5.309 | 5.309 | 0 | 0 | 0 |
| 3 | `Paineis` | `Codigo/Paineis` | 0 | 14 | 4.621 | 4.621 | 0 | 0 | 0 |
| 4 | `Geradores` | `Codigo/Geradores` | 1 | 16 | 3.757 | 3.757 | 0 | 0 | 0 |
| 5 | `ModulosMundo` | `Codigo/ModulosMundo` | 0 | 12 | 3.651 | 3.651 | 0 | 0 | 0 |
| 6 | `Prefabs` | `Codigo/Prefabs` | 0 | 15 | 3.256 | 3.256 | 0 | 0 | 0 |
| 7 | `ModulosGerais` | `Codigo/ModulosGerais` | 0 | 11 | 2.474 | 2.474 | 0 | 0 | 0 |
| 8 | `ModulosBatalha` | `Codigo/ModulosBatalha` | 0 | 8 | 1.026 | 1.026 | 0 | 0 | 0 |
| 9 | `Cenas` | `Codigo/Cenas` | 0 | 6 | 917 | 917 | 0 | 0 | 0 |
| 10 | `Server` | `Codigo/Server` | 0 | 4 | 358 | 358 | 0 | 0 | 0 |

### Top 50 maiores arquivos `.py` por linhas

| Arquivo | Linhas | Tamanho (KiB) |
|---|---:|---:|
| `Codigo/Paineis/FichaPokemon.py` | 1.277 | 57.22 |
| `Outros/GeradorRelatorios.py` | 1.062 | 34.16 |
| `Codigo/Telas/Inventario/InventarioPokemons.py` | 1.061 | 48.38 |
| `Codigo/ModulosMundo/ControladorObjetos.py` | 809 | 39.99 |
| `SimuladorServerJogo/Controle/EstadoServidor.py` | 754 | 30.29 |
| `Codigo/ModulosMundo/ControladorPlayer.py` | 710 | 33.86 |
| `SimuladorServerJogo/Rotas/Comandos.py` | 686 | 28.52 |
| `Codigo/Geradores/PokemonMundo.py` | 685 | 33.75 |
| `Codigo/Paineis/Container.py` | 637 | 23.33 |
| `SimuladorServerJogo/Controle/BancoDados.py` | 601 | 28.48 |
| `Codigo/ModulosGerais/Sonoridades.py` | 553 | 14.59 |
| `Codigo/Prefabs/Texto.py` | 548 | 20.48 |
| `Codigo/Paineis/PainelArvoreHabilidades.py` | 547 | 26.81 |
| `SimuladorServerJogo/Rotas/Atualizador.py` | 518 | 24.62 |
| `Outros/EditorSkins.py` | 514 | 19.97 |
| `SimuladorServerJogo/Geradores/GeradorPokemon.py` | 509 | 20.36 |
| `Codigo/Telas/Inventario/InventarioItens.py` | 509 | 23.76 |
| `SimuladorServerJogo/Controle/Cerebros/CerebroNPCs.py` | 500 | 26.00 |
| `Codigo/Telas/TelaServers.py` | 492 | 15.67 |
| `Codigo/ModulosMundo/LeitorDialogo.py` | 478 | 19.33 |
| `Codigo/Prefabs/Botao.py` | 477 | 17.50 |
| `SimuladorServerJogo/Controle/ObjetosMundoServer.py` | 472 | 30.55 |
| `Outros/Mixer.py` | 460 | 15.31 |
| `Codigo/ModulosGerais/FiltroCamera.py` | 460 | 21.03 |
| `Codigo/Paineis/PainelReceitas.py` | 455 | 15.29 |
| `Codigo/Telas/TelasGenericas.py` | 448 | 15.30 |
| `Codigo/Telas/SubtelaDialogo.py` | 428 | 18.78 |
| `Codigo/Telas/SubtelaCriarPersonagem.py` | 423 | 15.31 |
| `Codigo/Geradores/PokemonBatalha.py` | 422 | 18.35 |
| `Codigo/Geradores/Ator.py` | 409 | 17.87 |
| `Codigo/Telas/Inventario/Estatisticas.py` | 401 | 17.83 |
| `SimuladorServerJogo/Controle/Cerebros/CerebroCentral.py` | 400 | 20.55 |
| `Codigo/Geradores/Player/Controle.py` | 400 | 17.10 |
| `SimuladorServerJogo/Geradores/GeradorMundo.py` | 385 | 15.69 |
| `Codigo/ModulosGerais/Colisor.py` | 379 | 14.48 |
| `Codigo/Telas/TelaOperador.py` | 379 | 13.53 |
| `Codigo/Cenas/CenaMundo.py` | 370 | 18.80 |
| `Codigo/Paineis/FichaAtaque.py` | 361 | 14.50 |
| `Codigo/Geradores/Estadio.py` | 358 | 13.38 |
| `Codigo/ModulosMundo/LeitorMundo.py` | 358 | 17.13 |
| `Codigo/Paineis/PainelTimes.py` | 356 | 13.45 |
| `Codigo/Paineis/PainelCraft.py` | 341 | 13.04 |
| `Codigo/ModulosMundo/Loja.py` | 336 | 15.88 |
| `SimuladorServerJogo/Controle/LoaderRegras.py` | 332 | 19.11 |
| `Codigo/Geradores/PokemonInventario.py` | 332 | 12.61 |
| `Codigo/ModulosBatalha/ControladorFluxos.py` | 327 | 14.45 |
| `Codigo/Prefabs/Painel.py` | 300 | 11.15 |
| `SimuladorServerJogo/Rotas/Ativador.py` | 297 | 13.56 |
| `Codigo/Paineis/FichaPokemonBatalha.py` | 279 | 14.28 |
| `Codigo/ModulosBatalha/InicializadorBatalha.py` | 263 | 11.57 |

### Top 15 maiores funções e métodos (linhas)

| Arquivo | Nome | Tipo | Linhas |
|---|---|---:|---:|
| `Codigo/Geradores/Estadio.py` | `EstadioInterno.renderizar` | metodo | 232 |
| `Outros/GeradorRelatorios.py` | `coletar_metricas` | funcao | 224 |
| `SimuladorServerJogo/Rotas/Atualizador.py` | `processar_atualizador_json` | funcao | 215 |
| `Codigo/Telas/Inventario/InventarioPokemons.py` | `InventarioPokemons.atualizar` | metodo | 147 |
| `Codigo/ModulosGerais/Colisor.py` | `Colisor.resolver_movimento_com_colisores` | metodo | 146 |
| `Codigo/Prefabs/Fluxos.py` | `Fluxo.desenhar` | metodo | 141 |
| `Codigo/Telas/TelaMenu.py` | `TelaMenu` | funcao | 131 |
| `Codigo/Telas/Inventario/InventarioPokemons.py` | `InventarioPokemons._reconstruir` | metodo | 118 |
| `Codigo/Prefabs/Botao.py` | `Botao.render` | metodo | 117 |
| `Codigo/Telas/SubtelaCriarPersonagem.py` | `SubtelaCriarPersonagem._rebuild_layout` | metodo | 112 |
| `Outros/Mixer.py` | `main` | funcao | 109 |
| `SimuladorServerJogo/Geradores/GeradorMundo.py` | `_executar_world_generator` | funcao | 106 |
| `SimuladorServerJogo/Controle/Cerebros/CerebroItensMundo.py` | `CerebroItensMundo.executar_tick` | metodo | 101 |
| `Codigo/Telas/TelaConfig.py` | `_montar_layout` | funcao | 99 |
| `SimuladorServerJogo/Rotas/Entrada.py` | `processar_entrada_json` | funcao | 98 |

### Top 15 maiores classes (linhas)

| Arquivo | Classe | Linhas |
|---|---|---:|
| `Codigo/Paineis/FichaPokemon.py` | `FichaPokemon` | 1.245 |
| `Codigo/Telas/Inventario/InventarioPokemons.py` | `InventarioPokemons` | 1.033 |
| `Codigo/ModulosMundo/ControladorObjetos.py` | `ControladorObjetos` | 788 |
| `Codigo/ModulosMundo/ControladorPlayer.py` | `ControladorPlayer` | 691 |
| `Codigo/Geradores/PokemonMundo.py` | `Pokemon` | 661 |
| `Codigo/Paineis/Container.py` | `Container` | 628 |
| `SimuladorServerJogo/Controle/BancoDados.py` | `BancoDadosMundo` | 573 |
| `Codigo/Paineis/PainelArvoreHabilidades.py` | `PainelArvoreHabilidades` | 517 |
| `Codigo/Telas/Inventario/InventarioItens.py` | `InventarioItens` | 492 |
| `SimuladorServerJogo/Controle/Cerebros/CerebroNPCs.py` | `CerebroNPCs` | 480 |
| `Codigo/ModulosMundo/LeitorDialogo.py` | `LeitorDialogo` | 468 |
| `Codigo/ModulosGerais/FiltroCamera.py` | `FiltroCamera` | 451 |
| `Codigo/Paineis/PainelReceitas.py` | `PainelReceitas` | 439 |
| `Codigo/Telas/SubtelaDialogo.py` | `SubtelaDialogo` | 412 |
| `Codigo/Geradores/PokemonBatalha.py` | `PokemonBatalha` | 406 |

### Top 5 arquivos `.py` com mais imports

| Arquivo | Imports | Linhas |
|---|---:|---:|
| `SimuladorServerJogo/Controle/Cerebros/CerebroCentral.py` | 25 | 400 |
| `Codigo/Paineis/FichaPokemon.py` | 22 | 1.277 |
| `Codigo/Telas/Inventario/InventarioPokemons.py` | 21 | 1.061 |
| `Codigo/Cenas/CenaMundo.py` | 19 | 370 |
| `SimuladorServerJogo/Controle/EstadoServidor.py` | 15 | 754 |

## Arquivos por linhas

### Top 10 arquivos `.json` por linhas

| Arquivo | Linhas |
|---|---:|
| `SimuladorServerJogo/Regras/Geracao.json` | 658 |
| `Dados/InteracoesNPC/ManualDialogos.json` | 283 |
| `Dados/InteracoesNPC/Vendedores/Josefa.json` | 265 |
| `Dados/InteracoesNPC/ExemploLider_Alleka.json` | 227 |
| `Dados/InteracoesNPC/ExemploDissociado_Nico.json` | 224 |
| `Dados/InteracoesNPC/ExemploCapitao_Laura.json` | 195 |
| `Dados/Pokemon Global Server - Receitas.json` | 188 |
| `Dados/InteracoesNPC/ExemploDesafiante_Ravi.json` | 169 |
| `Dados/InteracoesNPC/Combatentes/Alleka.json` | 76 |
| `Dados/InteracoesNPC/Combatentes/Amable.json` | 76 |

### Top 10 maiores arquivos por linhas

| Arquivo | Ext | Linhas |
|---|---:|---:|
| `SimuladorServerJogo/Geradores/WorldGenerator.java` | `.java` | 2.835 |
| `Dados/Pokemon Global Server - Pokemons.csv` | `.csv` | 1.277 |
| `Codigo/Paineis/FichaPokemon.py` | `.py` | 1.277 |
| `Outros/GeradorRelatorios.py` | `.py` | 1.062 |
| `Codigo/Telas/Inventario/InventarioPokemons.py` | `.py` | 1.061 |
| `Codigo/ModulosMundo/ControladorObjetos.py` | `.py` | 809 |
| `SimuladorServerJogo/Controle/EstadoServidor.py` | `.py` | 754 |
| `Codigo/ModulosMundo/ControladorPlayer.py` | `.py` | 710 |
| `SimuladorServerJogo/Rotas/Comandos.py` | `.py` | 686 |
| `Codigo/Geradores/PokemonMundo.py` | `.py` | 685 |

## Arquivos por tamanho

### Top 10 maiores arquivos (tamanho)

| Arquivo | Ext | Tamanho (KiB) |
|---|---:|---:|
| `Recursos/Sonoridades/Musicas/Menu/Menu2.ogg` | `.ogg` | 4834.14 |
| `Recursos/Sonoridades/Musicas/Mundo/Praia.ogg` | `.ogg` | 4515.88 |
| `Recursos/Sonoridades/Musicas/Menu/Menu3.ogg` | `.ogg` | 4380.70 |
| `Recursos/Sonoridades/Musicas/Mundo/Vale.ogg` | `.ogg` | 3131.59 |
| `Recursos/Sonoridades/Musicas/Menu/Carregamento.ogg` | `.ogg` | 3124.37 |
| `Recursos/Sonoridades/Musicas/Menu/Menu1.ogg` | `.ogg` | 3064.16 |
| `Recursos/Sonoridades/Musicas/Mundo/Deserto.ogg` | `.ogg` | 2773.86 |
| `Recursos/Sonoridades/Musicas/Batalha/Confrontos/ConfrontoDaNeve.ogg` | `.ogg` | 2554.22 |
| `Recursos/Sonoridades/Musicas/Batalha/Confrontos/ConfrontoDoMar.ogg` | `.ogg` | 2525.50 |
| `Recursos/Sonoridades/Musicas/Batalha/Confrontos/ConfrontoDoVulcao.ogg` | `.ogg` | 2417.58 |

## Linhas por extensão

| Ext | Linhas |
|---:|---:|
| `.py` | 35.235 |
| `.json` | 3.729 |
| `.java` | 2.835 |
| `.csv` | 1.616 |
| `.toml` | 357 |
| `.md` | 236 |

## Top extensões por tamanho

| Ext | Tamanho (GiB) | Arquivos |
|---:|---:|---:|
| `.png` | 0.350 | 68.784 |
| `.ogg` | 0.045 | 17 |
| `.wav` | 0.004 | 9 |
| `.jpg` | 0.003 | 21 |
| `.py` | 0.001 | 140 |
| `.ttf` | 0.000 | 2 |
| `.csv` | 0.000 | 7 |
| `.mp3` | 0.000 | 3 |
| `.java` | 0.000 | 1 |
| `.json` | 0.000 | 28 |
| `.class` | 0.000 | 14 |
| `.md` | 0.000 | 1 |

## Diferenças vs último relatório

| Métrica | Anterior | Atual | Δ |
|---|---:|---:|---:|
| pastas | 1.184 | 1.183 | -1 |
| arquivos | 69.035 | 69.042 | 7 |
| tamanho_bytes | 0.404 GiB | 0.404 GiB | 0.000 GiB |
| linhas_totais_geral | 42.651 | 44.008 | 1.357 |
| py_arquivos | 136 | 140 | 4 |
| linhas_totais | 33.908 | 35.235 | 1.327 |
| classes_encontradas | 124 | 130 | 6 |
| funcoes_encontradas | 381 | 385 | 4 |
| metodos_encontrados | 1.548 | 1.622 | 74 |

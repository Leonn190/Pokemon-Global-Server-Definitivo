...
from Servidor.Batalha.AmbienteBatalhaServer import AmbienteBatalhaServer
from Servidor.Batalha.ColetorAcoes import ColetorAcoes
from Servidor.Batalha.ConstrutorLog import ConstrutorLog
from Servidor.Batalha.IDsBatalha import IDsBatalha
from Servidor.Batalha.InicializadorPartida import InicializadorPartida
from Servidor.Batalha.ResolvedorFlags import ResolvedorFlags
from Servidor.Batalha.BatalhaIA.ControladorIABoss import ControladorIABoss
from Servidor.Batalha.BatalhaIA.ControladorIA import ControladorIA
from Servidor.Batalha.RodadorTurno import RodadorTurno
from Servidor.Gerais.Geradores.GeradorPokemon import materializar_pokemon
...
class Partida:
    def __init__(self, id_partida: str | None = None, dados_inicializacao: dict | None = None):
        ...
        self.pokemons_por_lado: dict[int, list] = {}
        self.inicializador = InicializadorPartida(self)
        ...
        self.ambiente_batalha = AmbienteBatalhaServer(self)
        self.construtos = {}
    ...
    def _registrar_passivas_pokemon(self):
        try:
            from Servidor.Logica.Executes.ExecutesAtaques.ControladorExecutes import obter_passivas_ataque
        except Exception:
            return
    ...
    def _montar_areas(self):
        return self.inicializador.montar_areas()

    def _inicializar_lados(self, dados):
        return self.inicializador.inicializar_lados(dados)

    def _inicializar_inventarios(self, dados):
        return self.inicializador.inicializar_inventarios(dados)
    ...
    def _coletar_pokemons_iniciais(self, dados):
        return self.inicializador.coletar_pokemons_iniciais(dados)

    def _randomizar_posicoes_teste(self, pokemons):
        return self.inicializador.randomizar_posicoes_teste(pokemons)

    def _inicializar_pokemons(self, dados):
        return self.inicializador.inicializar_pokemons(dados)
    ...
    def mudar_clima(self, nome, origem=None, dados=None):
        return self.ambiente_batalha.mudar_clima(nome, origem=origem, dados=dados)

    def limpar_clima(self, motivo=None):
        return self.ambiente_batalha.limpar_clima(motivo=motivo)

    def aplicar_variacoes_temporarias_clima(self, pokemon):
        return self.ambiente_batalha.aplicar_variacoes_temporarias_clima(pokemon)

    def _pokemon_possui_ataque_code(self, pokemon, code):
        return self.ambiente_batalha._pokemon_possui_ataque_code(pokemon, code)

    def _parametro_ataque_code(self, code, chave, default=0.0):
        return self.ambiente_batalha._parametro_ataque_code(code, chave, default=default)

    def aplicar_clima_em_pokemon_por_passo(self, pokemon):
        return self.ambiente_batalha.aplicar_clima_em_pokemon_por_passo(pokemon)

    def aplicar_modificadores_dano_clima(self, tipo_ataque, dano):
        return self.ambiente_batalha.aplicar_modificadores_dano_clima(tipo_ataque, dano)

    def processar_fim_de_turno_clima(self):
        return self.ambiente_batalha.processar_fim_de_turno_clima()

    def processar_clima_por_passo(self):
        return self.ambiente_batalha.processar_clima_por_passo()

    def mudar_terreno(self, area_id, terreno, origem=None, dados=None):
        return self.ambiente_batalha.mudar_terreno(area_id, terreno, origem=origem, dados=dados)

    def limpar_terreno(self, area_id, motivo=None):
        return self.ambiente_batalha.limpar_terreno(area_id, motivo=motivo)

    def obter_terreno_area(self, area_id):
        return self.ambiente_batalha.obter_terreno_area(area_id)

    def aplicar_terreno_ao_entrar(self, pokemon, area_id, dados=None):
        return self.ambiente_batalha.aplicar_terreno_ao_entrar(pokemon, area_id, dados=dados)

    def aplicar_terreno_por_passo(self, pokemon):
        return self.ambiente_batalha.aplicar_terreno_por_passo(pokemon)

    def aplicar_variacoes_temporarias_terreno(self, pokemon):
        return self.ambiente_batalha.aplicar_variacoes_temporarias_terreno(pokemon)

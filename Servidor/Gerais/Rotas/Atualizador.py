...
from Servidor.Gerais.Rotas.Ativador import registrar_diff, diff_seq_atual, _obter_state_client, _coletar_diffs_visibilidade, _filtrar_pacotes_por_camera, _normalizar_posicao, _chunks_carregados_cliente, _raio_visao_por_regras
from Servidor.Mundo.BancoDados import BANCO_DADOS
from Servidor.Mundo.ObjetosMundoServer import AtorServer, PokemonServer, criar_objeto_mundo_server
from Servidor.Gerais.EstadoServidor import atualizar_perfil_personagem, atualizar_posicao_personagem, atualizar_inventario_personagem, aplicar_respawn_mundo, registrar_checkpoint_mundo_seguro, registrar_checkpoint_mundo_chunk_seguro, matar_player, player_invulneravel, aplicar_dano_player, aplicar_invulnerabilidade_player
...
from Servidor.Gerais.LoaderRegras import carregar_regras_batalha_publicas, carregar_regras_estruturas_naturais, carregar_regras_pokemons
from Servidor.Gerais.Geradores.GeradorPokemon import evoluir_pokemon, ganhar_xp_pokemon, materializar_pokemon, subir_nivel_pokemon
from Servidor.Mundo.EstadioGeometria import contexto_batalha_estadio, offset_porta_externa
from Servidor.Mundo.DungeonBatalha import contexto_batalha_dungeon
from Servidor.Mundo.DungeonGeometria import eh_dimensao_dungeon
    ...
    for diff in diffs:
        ...
        if tipo == "update" and objeto_id is not None:
            ...
            if "posicao" in payload_in:
                ...
                if isinstance(obj, AtorServer) and eh_dimensao_dungeon(dim_obj):
                    payload_in["posicao"] = CEREBRO._cerebro_dungeons.normalizar_posicao_player(obj, payload_in.get("posicao"))
                else:
                        ...
                        contexto.update(
                            {
                                ...
                                "dungeon": {
                                    ...
                                    "servos_pool": list(layout_dungeon.get("servos_pool") or []),
                                },
                            }
                        )
            ...
            if categoria == "pokemon_derrotado_batalha":
                ...
                if pokemon_id > 0:
                    ...
                    if isinstance(obj_atual, PokemonServer) and bool(getattr(obj_atual, "estado_extra", {}).get("boss", False)):
                        ...
                        obj_atual.estado_extra.pop("batalha_confirmada", None)

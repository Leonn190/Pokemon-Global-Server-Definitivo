...
from Servidor.Gerais.Geradores.GeradorDungeons import gerar_dungeon_layout
from Servidor.Gerais.Geradores.GeradorPokemon import gerar_pokemon_server, materializar_pokemon
...
from Servidor.Mundo.Dungeons.EstadoDungeon import (
    criar_estado_entrada,
    limpar_estado_temporario,
    registrar_sala_explorada,
    resolver_posicao_saida_dungeon,
)
from Servidor.Mundo.ObjetosMundoServer import AtorServer, ItemMundoServer, PokemonServer
from Servidor.Mundo.ServicoInventario import ServicoInventario
from Servidor.Mundo.CerebroArmadilhas import CerebroArmadilhas
...
class CerebroDungeons:
    def __init__(self, cerebro_central):
        ...
        self._layouts = {}
        self._cerebro_armadilhas = CerebroArmadilhas()
    ...
    def entrar_dungeon(self, client_id, pedra_id, porta_idx, dungeon_code, registrar_diff=None):
        ...
        entradas_dungeon = player.estado_extra.get("entradas_dungeon_mundo") if isinstance(player.estado_extra.get("entradas_dungeon_mundo"), dict) else {}
        entradas_dungeon[f"{code_real}:{int(porta_real)}:{int(pedra_id or 0)}"] = list(entrada_mundo)
    ...
    def sair_dungeon(self, client_id, registrar_diff=None):
        ...
        if isinstance(entrada, dict) and not bool(entrada.get("ativa", True)):
            return False
    ...
    def executar_tick(self, chunks_carregados, chunks_simulados, registrar_diff):
        ...
        for dimensao, players in players_por_dim.items():
            ...
            alterou_tiles = self._cerebro_armadilhas.executar_tick(
                layout,
                players,
    ...
    def _players_dungeon_por_dimensao(self):
        ...
        for obj in BANCO_DADOS.listar_objetos():
            if not isinstance(obj, AtorServer):
                continue
            if bool(obj.estado_extra.get("morto", False) or obj.estado_extra.get("game_over", False)):
                continue
    ...
    def _garantir_servos(self, layout, salas_por_id, registrar_diff):
        ...
        for item in list(layout.get("servos") or []):
            ...
            if not isinstance(sala, dict):
                continue
            pos = item.get("posicao") if isinstance(item.get("posicao"), (list, tuple)) and len(item.get("posicao")) == 2 else self._posicao_spawn_sala(sala)
    ...
    def _posicao_proibida_servo(self, layout: dict, sala: dict, x: float, y: float, raio: float = 0.45, ignorar_id: int = 0) -> bool:
        ...
        for trap in list(cfg.get("armadilhas") or []):
            ...
            if not bool(tcfg.get("solido", False) or tcfg.get("solido_centro", False)):
                continue
            if str(trap.get("tipo") or "") == "espeto_movel":
                continue
    ...
    def _spawn_pokemon_dungeon(self, especie, pos, sala, layout, tipo, registrar_diff, servo_info=None):
        ...
        if isinstance(servo_info, dict) and int(servo_info.get("nivel", 0) or 0) > 0:
            bruto = dict(poke.estado_extra)
            bruto["nivel"] = int(servo_info.get("nivel", 1) or 1)
            mat = materializar_pokemon(bruto)
            estado_mat = mat.get("estado") if isinstance(mat.get("estado"), dict) else mat
            if isinstance(estado_mat, dict):
                poke.estado_extra.update(estado_mat)

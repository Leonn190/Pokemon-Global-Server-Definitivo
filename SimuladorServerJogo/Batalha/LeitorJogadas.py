...
class LeitorJogadas:
    ...
    def _interpretar_ataque(self, sistema: SistemaBatalha, ataque: Dict[str, object] | None) -> Dict[str, object]:
        ...
        if not spec["dano_componentes"] and estilo in {"movimento", "area", "tiro", "alvo"}:
    ...
    def _ordenar_jogadas(self, sistema: SistemaBatalha, jogadas: List[Dict[str, object]]) -> List[Dict[str, object]]:
        normalizadas = []
        for indice, jogada in enumerate(list(jogadas or [])):
            ...
            if executor is None or executor.ForaDeCombate:
                continue
    ...
    def _calcular_pacote_dano(self, sistema: SistemaBatalha, atacante, alvo, spec: Dict[str, object], fluxo: Dict[str, object], tick: int) -> Dict[str, object]:
        ...
        dano_final = max(0.0, dano_defendido * mod_tipo)
    ...
    def _processar_movimento_ativo(self, sistema: SistemaBatalha, movimento: Dict[str, object], log: Dict[str, object], tick: int) -> None:
    ...
    def executar_turno(self, sistema: SistemaBatalha, client_id: str, jogadas: List[Dict[str, object]] | None = None) -> Dict[str, object]:
        ...
        ordenadas = self._ordenar_jogadas(sistema, jogadas_brutas)
        log = {
            ...
            "eventos": [],
            "eventos_por_tick": {},
            "snapshot_inicial": sistema.snapshot(),
        }
        ...
        while tick < limite and (pendentes or ativos_movimento or objetos_ativos or acertos_alvo):
            ...
            for movimento in list(ativos_movimento):
                if not movimento.get("ativo", False):
                    ...
                    continue
                self._processar_movimento_ativo(sistema, movimento, log, tick)
                if not movimento.get("ativo", False):
                    ativos_movimento.remove(movimento)
        ...
        log["snapshot_final"] = sistema.snapshot()
        sistema.avancar_turno(log)
        return {"status": "ok", "mensagem": "Turno computado", "batalha": sistema.snapshot()}

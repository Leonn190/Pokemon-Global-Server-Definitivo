...
def _cmd_tp(autor, args):
    ...
    if not alvos:
        return "Erro no /tp. Ordem base: /tp alvo posx posy"
    largura, altura = BANCO_DADOS.limites_mundo()
    px, py = float(x) % max(1.0, float(largura)), float(y) % max(1.0, float(altura))
    for alvo in alvos:
        atualizar_posicao_personagem(alvo, [px, py])
        players = _estado_players()
        skin = str((players.get(alvo, {}) if isinstance(players.get(alvo, {}), dict) else {}).get("skin", "Verde"))
        BANCO_DADOS.garantir_player(alvo, skin, (px, py))
    alvo_txt = "todos" if len(alvos) > 1 else alvos[0]
...
def _cmd_spawn(autor, args):
    ...
    chunk = BANCO_DADOS.chunk_da_posicao((float(x), float(y)))
    especie_ref = str(poke.get("Code") or poke.get("Nome") or "").strip()
    obj = GERADOR_POKEMON_SERVER.gerar(novo_id=novo_id, posicao=(float(x), float(y)), chunk_xy=chunk, especie=especie_ref)
    _ajustar_stats_spawn(obj.estado_extra, nomeados)
    BANCO_DADOS.inserir_objeto(obj)
    registrar_diff("spawn", payload=obj.serializar(), escopo={"centro": [obj.posicao[0], obj.posicao[1]], "raio": 80}, objeto_id=obj.Id, categoria="rapida")
...
def _cmd_chest(autor, args):
    ...
    novo_id = BANCO_DADOS.gerar_id()
    bau = BauServer(id_objeto=novo_id, tipo_bau=tipo, itens=list(dados.get("itens", [])), posicao=(float(x), float(y)), raio_colisao=0.42, raio_interacao=0.85, aberto=False)
    BANCO_DADOS.inserir_objeto(bau)
    registrar_diff("spawn", payload=bau.serializar(), escopo={"centro": [bau.posicao[0], bau.posicao[1]], "raio": 80}, objeto_id=bau.Id, categoria="rapida")
...
def executar_comando_terminal(autor: str, texto: str) -> dict:
    ...
    try:
        ...
        ordem = {"give": "/give alvo item quantidade", "tp": "/tp alvo posx posy", "spawn": "/spawn pokemon posx posy", "chest": "/chest tipo posx posy"}.get(cmd, f"/{cmd}")
